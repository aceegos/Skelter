...
function legal(c,who){
 if(c.r==="JOKER")return true;
 var active=wildSuit||tableCard.s;
 if(court(c))return house===who&&c.s===active;
...
function flash(x){var f=el("flash");f.textContent=x;f.style.display="flex";setTimeout(function(){f.style.display="none"},900)}
function render(){
...
function finish(who,c,keepWild){
 ...
 if(c.r==="A"||c.r==="10"){house=who;flash(who==="human"?"YOU HAVE THE HOUSE":"COMPUTER HAS THE HOUSE")}
 ...
 if(turn==="computer"){message("Computer turn");setTimeout(computerTurn,3000)}else message("Your turn");
}
...
function computerTurn(){
 ...
 if(c.r==="JOKER"){var counts={S:0,H:0,D:0,C:0};cpu.forEach(function(x){if(x.s)counts[x.s]++});wildSuit="S";suits.forEach(function(s){if(counts[s]>counts[wildSuit])wildSuit=s});flash("COMPUTER CHOOSES "+symbols[wildSuit]);finish("computer",c,true)}
